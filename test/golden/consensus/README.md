...
| What is a *corroborating* reporter in §7's `high`? | The estimate needs **two** things sitting on it, both within five minutes: a log-line report, and a second, different reporter. A reporter ten minutes off corroborates nothing — and neither does a second *log line* ten minutes off, which is the disagreement `wide_spread` reports. `confidence_log_line_without_corroborator_is_medium` and `confidence_two_distant_log_lines_is_medium` versus `log_line_median_only` |
