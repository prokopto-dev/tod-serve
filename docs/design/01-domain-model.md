...
| `instance_setting_change` | append-only | One change to an instance-wide policy switch: which `setting`, its `old_value` and `new_value`, who and when. Hash-chained, and its own audit record for the same reason `instance_grant` is. [ADR-0018](../adr/0018-instance-settings-are-mutable-with-a-change-ledger.md). `public_url` is not a value `setting` can hold — it is immutable over the API, because it must keep matching every registered redirect URI. |
