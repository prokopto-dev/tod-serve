...
| B — Per-instance, operator-registered application | Both blockers close outright: no shared app to violate anyone's terms, and a token minted for the operator's own client id is worthless elsewhere. Guild scopes become available | Every operator must register a Discord application and paste a client id and secret before `discord` works at all. The instance now stores a `client_secret` at rest |
...
The callback exchanges the code, calls `users/@me` and `users/@me/guilds`, **discards the Discord
access token**, writes the ticket and redirects. `credential.kind` gains `provider_ticket`;
[ADR-0007](0007-one-join-endpoint.md)'s one-join-endpoint rule is preserved exactly — `/join` and
`/sessions` still take one credential union.

Mechanisms: `TestDiscord_AccessToken_NeverPersisted`,
`TestCredentialTicket_SecondRedemption_Refused`, `TestCredentialTicket_After120s_Refused`,
`TestGuildGate_EvaluatedOnJoinAndSessions`.

### Consequences

- Good, because both release blockers close outright rather than being mitigated, and
  `credential_stale` stops being load-bearing.
