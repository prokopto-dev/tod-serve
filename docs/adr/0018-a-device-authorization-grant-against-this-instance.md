...
([ADR-0019](0019-a-device-grant-is-identity-scoped.md)). The two that accept a code join the
...
- **Bad, because a client holds one token per circle** (ADR-0019), which every client author will
  get wrong once.
