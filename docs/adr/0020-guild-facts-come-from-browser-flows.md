# ADR-0020 — Guild facts for a device grant come from browser flows, not a stored Discord credential

**Status:** proposed · **Date:** 2026-08-31 · **Deciders:** Courtney Caldwell

## Context and problem statement

[ADR-0019](0019-a-device-grant-is-identity-scoped.md)'s exchange re-proves a circle's guild gate
...
ADR-0019's "circles appear automatically" is false precisely where the circle gates — which is the
...
Exchange reads a row only past ADR-0019's live gate re-read and only while `verified_at` is within
`GateFactsTTL`.
