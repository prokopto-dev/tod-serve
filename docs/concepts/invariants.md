...
| A `provider_ticket` never appears in a URL query | It reaches the SPA in the redirect **fragment** (`<spa>/join#ticket=…`), which no browser transmits, and is cleared immediately. `TestNoTokenInURL_CallbackRedirectUsesFragment` | Phase 1 |
