...
| **First-run setup** | The window in which a caller holding `TOD_SETUP_TOKEN` may create the first circle, provider and owner code. It is open exactly while **no identity administers the instance** — derived, never a stored flag — and closes for good at the first administrator. See [ADR-0016](../adr/0016-first-run-setup-is-an-env-token-and-a-derived-window.md). |
| **Capability floor** | The permissions that are **session-and-step-up only**: operations altering authentication, authorization or bulk-export state, which **no PAT reaches at any scope**. A leaked token cannot mint another token, revoke a member or read the audit log. Generated into [permissions.md](../reference/permissions.md); [ADR-0012](../adr/0012-instance-grants-are-a-capability-ledger.md). |
