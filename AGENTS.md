...
| `internal/invite/` | Invite codes: minting, the generous parser a hand-typed code needs, **the one hash and the one owner-grant lookup** `identitysql` is handed, and the single-use owner grant that gives a circle its first owner. A grant is not an `invite` row, so every path that resolves a pasted code needs both rungs — the OAuth flow had one, and refused every first-run owner code |
